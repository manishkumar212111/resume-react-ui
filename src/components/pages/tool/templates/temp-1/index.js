...
const Index = (props) => {
    ...
    const renderAwards = () => {
        ...
        resume_detail.awards && resume_detail.awards.map((itm,index)=> h.push(
            <p className="resumeAwardRec">{itm.title}</p>
        ))  
        return h;
    ...
    return(
         <main class={`${props.downloads ? "downloadMain" : ""} resumeMain ${resume_detail.style.fontSize == 12 ? "smallTxt" : (resume_detail.style.fontSize == 14 ? "mediumTxt" : (resume_detail.style.fontSize == 16 ? "largeTxt" : "extraLargeTxt"))}`} style={{backgroundColor : resume_detail?.style?.background, color: resume_detail?.style?.text_color}}>
        <div className="resumeLeft">
            <div className="resumeProfile">
                {resume_detail.profileImage && <div className="resumePP"><img src={resume_detail.profileImage} alt=""/></div>}
                <ul>
        ...
        <div className="resumeRight">
            {basic_info && <h1>{basic_info.first_name} <strong>{basic_info.last_name}</strong></h1>}
            <div className={"resumePosTitle"}>
            {props?.resume_detail?.job_title ?  props.resume_detail.job_title :"Job Title"}
            </div>
